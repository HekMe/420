...
function initialize() {
  ...
  if(getCookie("GUID") != ""){
    var counter = setInterval(()=>{
      var web = "https://4-2-0.life/count.txt";
      fetch(web)
      .then((response) => response.text().then(cb));
    
      function cb( txt ) { document.getElementById("join_joint").innerText = `💨 ${txt.toString().trim()} smokers jointed❗💨`;}
    },1000);  
  }
...
function getTzTime(){
  ...
  tzTime = setInterval(()=>{
    ...
    if(moment.tz(getCookie("tz")).format("hh:mm:ss a").indexOf("04:19:45") === 0){setupRain()};
  }, 100)
}
